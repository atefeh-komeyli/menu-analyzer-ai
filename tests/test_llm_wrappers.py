...
class TestExtractMenuItems:
    def test_extract_menu_items_success(self, sample_image_list, mock_openai):
        ...
        sample_menu_items = [
            {"name": "Pasta Carbonara", "description": "Pasta with eggs, cheese, pancetta, and pepper", "price": "$12.99"},
            {"name": "Caesar Salad", "description": "Romaine lettuce with Caesar dressing and croutons", "price": "$8.99"}
        ]
        
        # Mock the LLM response
        mock_instance = mock_openai.return_value
        mock_instance.invoke.return_value = AIMessage(content=json.dumps(sample_menu_items))
...
class TestGenerateNextQuestion:
    def test_generate_next_question(self, mock_openai):
        ...
        # Mock response
        mock_instance = mock_openai.return_value
        mock_instance.invoke.return_value = AIMessage(content="Would you prefer a vegetarian option?")
    ...
    def test_generate_next_question_no_question_mark(self, mock_openai):
        ...
        mock_instance = mock_openai.return_value
        mock_instance.invoke.return_value = AIMessage(content="Tell me about your dietary preferences")
